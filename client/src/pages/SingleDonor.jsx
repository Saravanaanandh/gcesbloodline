...
const SingleDonor = () => {
    ...
    // Auto-show forced popup on page load if user hasn't filled recipient form
    // Wait for auth check to complete to avoid false positive on initial render
    useEffect(() => {
        if (!isCheckAuth && !isUserAsRecipient) {
            setForcedModal(true);
        } else if (!isCheckAuth && isUserAsRecipient) {
            setForcedModal(false);
        }
    }, [isCheckAuth, isUserAsRecipient]);

    const handleSendRequest = async (id, donorId) => {
        // Guard: user must have filled the recipient form
    ...
    return (
        <div className="min-h-[100vh]">
            {/* Forced page-load popup when recipient form not filled */}
            <FormRequiredModal
                isOpen={forcedModal}
                onClose={() => { }}
                userType="recipient"
                forced={true}
            />
            {/* Action-triggered popup (clicking Send Request without recipient form) */}
            <FormRequiredModal
